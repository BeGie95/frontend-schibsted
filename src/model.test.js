...
describe('model tests', () => {
  ...
  it('should init model with proper state', () => {
    expect(model.getData({})).toEqual({ articles: [], error: null });
  });

  it('should properly set error state', () => {
    model.setError({ error: '500 server error' });
  ...
  it('should properly set dateUnix', () => {
    ...
    model.setArticles({ articles: fakeArticlesData });
    model
      .getData({
        filters: {
          fashion: true,
      ...
      .articles.forEach((element, index) => {
        expect(element.dateUnix).toBe(unixDatesArray[index]);
      });
  });
});
